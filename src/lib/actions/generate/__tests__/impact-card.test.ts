...
import { generateImpactStatementAction } from '../impact-card';
...
describe('Impact Card Server Actions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('generateImpactStatementAction', () => {
    const validInput = {
      inputText: 'Our new AI tool helps developers ship features 10x faster with zero boilerplate.',
    };
    ...
    it('returns error if user is unauthenticated', async () => {
      ...
      const result = await generateImpactStatementAction(validInput);
    ...
    it('successfully extracts 3 impact statements and creates Generation record', async () => {
      const { auth } = await import('@/lib/auth');
      const { visualExportRateLimit } = await import('@/lib/ratelimit');
      const { prisma } = await import('@/lib/prisma');
      ...
      const mockStatements = [
        ...
        'The future of rapid AI development is here.',
      ];
      ...
      if (result.success) {
        expect(result.data.statements).toEqual(mockStatements);
