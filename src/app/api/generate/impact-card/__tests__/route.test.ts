...
describe('POST /api/generate/impact-card', () => {
  ...
  it('returns 400 if ANTHROPIC_API_KEY is missing', async () => {
    ...
    const { POST } = await import('../route');
    const req = createMockRequest({ inputText: 'Some content' });
  ...
  it('uses BYOK decrypted key via resolveAnthropicModel for tester role', async () => {
    ...
    vi.mocked(generateObject).mockResolvedValueOnce({ object: { statements: ['S1'] } } as any);

    const { POST } = await import('../route');
    const req = createMockRequest({ inputText: 'Tester content' });
  ...
  it('returns error response when resolveAnthropicModel returns a decryption error', async () => {
    ...
    const { POST } = await import('../route');
    const req = createMockRequest({ inputText: 'Bad key content' });
