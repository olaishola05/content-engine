...
`;
    ...
    const result = await streamObject({
      ...
      prompt: `Please repurpose the following original content into the requested platforms and tone:\n\n${inputText}`,
      onFinish: async ({ object }) => {
        try {
          if (!object?.outputs) return;

          // Asynchronously save input and output to prisma DB
