...
vi.mock('../tiktok-client', () => ({
  default: () => 'TikTokClientComponent',
}));
...
describe('TikTokCarouselGeneratePage Server Component', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('redirects to /sign-in if user is unauthenticated', async () => {
    ...
    const { default: TikTokGeneratePage } = await import('../page');

    await expect(TikTokGeneratePage()).rejects.toThrow('Redirect to /sign-in');
  });
  ...
  it('redirects to /onboarding if user needs onboarding', async () => {
    ...
    await expect(TikTokGeneratePage()).rejects.toThrow('Redirect to /onboarding');
  });
  ...
  it('renders without throwing redirect if user is authenticated', async () => {
    ...
    const result = await TikTokGeneratePage();
