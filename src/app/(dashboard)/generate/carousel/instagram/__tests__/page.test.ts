...
vi.mock('../instagram-client', () => ({
  default: () => 'InstagramClientComponent',
}));
...
describe('InstagramCarouselGeneratePage Server Component', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('redirects to /sign-in if user is unauthenticated', async () => {
    ...
    const { default: InstagramGeneratePage } = await import('../page');

    await expect(InstagramGeneratePage()).rejects.toThrow('Redirect to /sign-in');
  });
  ...
  it('redirects to /onboarding if user needs onboarding', async () => {
    ...
    await expect(InstagramGeneratePage()).rejects.toThrow('Redirect to /onboarding');
  });
  ...
  it('renders without throwing redirect if user is authenticated', async () => {
    ...
    const result = await InstagramGeneratePage();
