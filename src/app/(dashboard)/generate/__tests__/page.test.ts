...
vi.mock('../generate-client', () => ({
  default: () => 'GenerateClientComponent',
}));

describe('GeneratePage Server Component', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('redirects to /sign-in if user is unauthenticated', async () => {
    ...
    await expect(GeneratePage()).rejects.toThrow('Redirect to /sign-in');
  });
